...
async fn oneconfig(data: Data<structs::AppState>, path: Path<(String, String)>) -> impl Responder {
	...
	let (latest_release, latest_release_time) =
		check_internal_error!(latest_release, "Unable to parse latest release from maven");

	// Fetch maven snapshots data
	let maven_snapshots_text: Result<String, reqwest::Error> = try {
		client
			.get(format!(
				"{0}/snapshots/cc/polyfrost/oneconfig-{version}-{loader}/maven-metadata.xml",
				data.internal_maven_url
			))
			.send()
			.await?
	...
	let (latest_snapshot, latest_snapshot_time) = check_internal_error!(
		latest_snapshot,
		"Unable to parse latest snapshot from maven"
	);
	...
	let (latest_loader, _) =
		check_internal_error!(latest_loader, "Unable to parse latest loader from maven");
	...
	let releases_info = structs::MavenDataResponse {
		url: release_url,
		sha256: check_internal_error!(release_sha),
	};
	...
	HttpResponse::Ok().json(structs::OneconfigDataResponse {
...
async fn main() -> std::io::Result<()> {
	let port = env::var("PORT")
		...
		.parse::<u16>()
		.expect("Unable to convert PORT variable to number");
			...
			.app_data(
				...
				Data::new(structs::AppState {
					...
					public_maven_url: env::var("PUBLIC_MAVEN_URL")
						.unwrap_or_else(|_| "https://repo.polyfrost.cc".to_string()),
